    ...
    for epoch in tqdm(range(1, epochs + 1), desc="Epoch...",
        ...
        with tqdm(total=steps_per_epoch, desc="Training...",
            ...
            for j in range(test_batch_num) :   
                batch_data = {k : test_ds[k][j*val_batch_size : (j+1)*val_batch_size] for k in test_ds.keys()} 
            ...
            if len(test_ds['image']) > test_batch_num*val_batch_size : 
                batch_data = {k : test_ds[k][test_batch_num*val_batch_size:] for k in test_ds.keys()} 
                valid_loss, outputs = validate(batch_data['image'], batch_data['label'] , model_state)
                for k,v in valid_loss.items() : 
                    valid_losses[k].append(v)
                valid_outputs.append(outputs) 
