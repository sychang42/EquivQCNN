...
class U2_ver1_oc(qml.operation.Operation):
    num_wires = 2
    num_params = 6 #int: Number of trainable parameters that the operator depends on.
    ...
    def compute_decomposition(*angle, wires):  # pylint: disable=arguments-differ
        ...
        op_list.append(qml.IsingZZ(angle[0], wires = wires)) 
        op_list.append(qml.RX(angle[1], wires = wires[0])) 
        op_list.append(qml.RX(angle[2], wires = wires[1])) 
        
        op_list.append(qml.IsingYY(angle[3], wires = wires)) 
...
class U2_ver2ZZ(qml.operation.Operation):
    num_wires = 2
...
class U2_ver2YY(qml.operation.Operation):
    num_wires = 2
    num_params = 3 #int: Number of trainable parameters that the operator depends on.
...
class U2_ver2YYZZ(qml.operation.Operation):
    num_wires = 2
    num_params = 3 #int: Number of trainable parameters that the operator depends on.
